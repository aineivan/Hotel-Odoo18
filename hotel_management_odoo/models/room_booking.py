...
class RoomBooking(models.Model):
    ...
    duration = fields.Integer(
        ...
        store=True,
        help="Calculated duration between check-in and check-out."
    )

    all_day_check = fields.Boolean(default=True)

    calendar_description = fields.Char(
        string="Calendar Details",
        compute='_compute_calendar_description'
    )
    ...
    checkin_date = fields.Datetime(
        ...
        store=True,
        readonly=False,
        help="Earliest check-in date from room lines."
    )
    checkout_date = fields.Datetime(
        ...
        store=True,
        readonly=False,
        help="Latest check-out date from room lines."
    )
    ...
    def _compute_booking_dates(self):
        for rec in self:
            if rec.room_line_ids:
                checkin_list = rec.room_line_ids.mapped('checkin_date')
                checkout_list = rec.room_line_ids.mapped('checkout_date')

                checkin_list = [d for d in checkin_list if d]
                checkout_list = [d for d in checkout_list if d]

                if checkin_list:
                    rec.checkin_date = min(checkin_list)
                if checkout_list:
                    rec.checkout_date = max(checkout_list)
            else:
                rec.checkin_date = fields.Datetime.now()
                rec.checkout_date = fields.Datetime.now() + timedelta(hours=23, minutes=59)
    ...
    @api.depends('partner_id', 'room_line_ids', 'amount_total')
    def _compute_calendar_description(self):
        for rec in self:
            rooms = ", ".join(rec.room_line_ids.mapped(
                'room_id.name')) or "No Room"
            symbol = rec.currency_id.symbol or ''
            rec.calendar_description = (
                f"👤 {rec.partner_id.name} | "
                f"🛏️ {rooms} | "
                f"💰 {symbol}{rec.amount_total}"
            )
